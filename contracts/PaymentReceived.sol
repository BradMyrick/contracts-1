...
contract PaymentReceived is ReentrancyGuard {
    ...
    modifier onlyNftCreator(address nftContract) {
        require(
            ...
            "Only the collection creator can perform this action"
        );
        _;
    }
    ...
    function authorize(address _contract)
        external
        nonReentrant
        onlyNftCreator(_contract)
        returns (bool _success)
    {
        require(_contract != address(0), "PaymentReceived: INVALID_ADDRESS");
        require(
            paymentToken.allowance(msg.sender, address(this)) >= assemblerPrice,
            "PaymentReceived: INSUFFICIENT_TOKEN_ALLOWANCE"
        );
