...
// constructor
    ...
    function createAuction(uint _endTime, bool _buyNow, uint _directBuyPrice,uint _startPrice,address _nftAddress,uint _tokenId) external nonReentrant() returns (bool){
        require(_directBuyPrice >= 1000000000000000 wei, "Direct buy price must be greater than 1000000000000000 wei"); //
        ...
        Auction auction = new Auction(msg.sender, _endTime, _buyNow, 1000000000000000 wei, _directBuyPrice, _startPrice, _nftAddress, _tokenId); // create the auction
        ...
        if (!collectionPresent[_nftAddress]) { // if the collection is not present
    ...
    function getOneNFT(address _NFT, uint _tokenId) external view returns(address _info) {
        ...
        require(NFTcollections[_NFT].auctionIndex[_tokenId] != address(0), "Token is not for sale");
   ...
   // get all for sale for collection
    ...
    function withdrawFunds(uint _amount, address _account) external nonReentrant() {
    ...
    function auctionStateChanged(uint256 _state) external {
        ...
        if (_state == 1 || _state == 3 || _state == 4) { // if the state is 1, 3 or 4 (auction is canceled, auction is direct buy or auction is auction buy)
    ...
    function _removeAuction(address _auction) internal {
        ...
        NFTcollections[Auction(_auction).nftAddress()].collection.remove(_auction); // remove the auction from the collection
        NFTcollections[Auction(_auction).nftAddress()].auctionIndex[Auction(_auction).tokenId()] = address(0); // remove the auction from the auction index
    ...
    function changeCaller(address _newCaller) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "You are not an admin"); // only the admin can change the caller
    ...
    function transferAuctionFees(address _to, address _auction) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender) || msg.sender == caller, "You are not an admin or the caller"); // only the admin or the caller can transfer the auction fees
        require(Auction(_auction).transferFee(_to), "Auction failed to transfer funds"); // require that the auction transfer the funds
        }

// events
    event AuctionState(address indexed auction, uint  indexed state);
    event collectionAdded(address indexed collection); // Event for when a collection is added
    event tokenAdded(address indexed collection, uint indexed tokenId); // Event for when a token is added
    event auctionCreated(address indexed auction); // Event for when an auction is created
    event managerFundsWithdrawn(uint indexed amount); // Event for when funds are withdrawn
